...
const katex_commands = [
    ...
    # paired delimiters
    lxd("norm", 1, raw"""\lVert #1 \rVert"""),
    lxd("fro", 1, raw"""\lVert #1 \rVert_{\operatorname{FRO}}"""),
    lxd("card", 1, raw"""\lvert #1 \rvert"""),
    lxd("abs", 1, raw"""\lvert #1 \rvert"""),
    lxd("inner", 2, raw"""\langle #1, #2 \rangle"""),
    lxd("floor", 1, raw"""\lfloor #1 \rfloor"""),
    lxd("ceil", 1, raw"""\lceil #1 \rceil"""),
    # operators
    ...
    lxd("std", 0, raw"""\sigma"""),
    lxd("Var", 1, raw"""\mathbb{V}\text{ar}[#1]"""),
    lxd("Cov", 2, raw"""\mathbb{C}\text{ov}[#1, #2]"""),
    lxd("Corr", 2, raw"""\mathbb{C}\text{orr}[#1, #2]"""),
    lxd("Entropy", 1, raw"""\mathbb{H}[#1]"""),
    lxd("MI", 2, raw"""\mathbb{I}[#1; #2]"""),
